...
fn test_new() {
    ...
    assert!((atomic.get() - std::f64::consts::PI).abs() < EPSILON);
}
...
fn test_from() {
    ...
    assert!((atomic.get() - std::f64::consts::E).abs() < EPSILON);
}

#[test]
fn test_get_set() {
    let atomic = AtomicF64::new(0.0);
    atomic.set(std::f64::consts::PI);
    assert!((atomic.get() - std::f64::consts::PI).abs() < EPSILON);
    atomic.set(-2.5);
...
fn test_compare_and_set_weak_in_loop() {
    ...
    for i in 0..10 {
        loop {
            match atomic.compare_and_set_weak(current, (i + 1) as f64) {
                Ok(_) => break,
...
#[test]
fn test_concurrent_update_and_get_extreme_contention() {
    let atomic = Arc::new(AtomicF64::new(0.0));
    let mut handles = vec![];
