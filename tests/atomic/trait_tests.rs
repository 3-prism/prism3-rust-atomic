...
fn test_atomic_trait_floats() {
    fn test_atomic<T: Atomic<Value = f32>>(atomic: &T) {
        ...
        assert!((atomic.get() - std::f32::consts::PI).abs() < 1e-6);
...
fn test_atomic_trait_via_generic<T: Atomic<Value = i32>>(atomic: &T) {
    ...
    assert!(atomic.compare_and_set(20, 30).is_ok());
...
// Test UpdatableAtomic trait through generic function
fn test_updatable_atomic_trait_via_generic<T: UpdatableAtomic<Value = i32>>(atomic: &T) {
    // Test trait methods through generic constraint
    let old = atomic.get_and_update(|x| x + 10);
    assert_eq!(old, 0);
...
// Test AtomicInteger trait through generic function
fn test_atomic_integer_trait_via_generic<T: AtomicInteger<Value = i32>>(atomic: &T) {
    // Test trait methods through generic constraint
...
fn test_all_traits_via_generic() {
    ...
    let atomic = AtomicI32::new(0);
    test_updatable_atomic_trait_via_generic(&atomic);

    let atomic = AtomicI32::new(0);
    test_atomic_integer_trait_via_generic(&atomic);
}
...
fn test_from_trait() {
    ...
    assert!((atomic_f32.get() - std::f32::consts::PI).abs() < 1e-6);
}
